...
std::optional<Json::Value> checkMetadataKeys(Json::Value const& _input)
{
	if (_input.isObject())
	{
		if (_input.isMember("append") && !_input["append"].isBool())
			return formatFatalError("JSONError", "\"settings.metadata.append\" must be Boolean");
	...
	static set<string> keys{"append", "useLiteralContent", "bytecodeHash"};
...
std::variant<StandardCompiler::InputsAndSettings, Json::Value> StandardCompiler::parseInput(Json::Value const& _input)
{
	...
	ret.metadataFormat =
		metadataSettings.get("append", Json::Value(true)).asBool() ?
