...
void interpret(string const& _source, bool _inspect)
{
	...
	try
	{
		...
		if (_inspect)
			InspectedInterpreter::run(std::make_shared<Inspector>(_source, state), state, dialect, *ast, /*disableMemoryTracing=*/false);
...
Allowed options)",
	...
	options.add_options()
		...
		("interactive", po::value<bool>(), "Run interactive")
	...
	else
	{
		...
		interpret(input, arguments.count("interactive"));
	}
